...
const authenticateUser = async (req, res, next) => {
  ...
  if (!token) {
    console.log("authentication didnt find the token");
    throw new CustomError.UnauthenticatedError("No token found");
  }

  try {
    const { name, userId, email, role, theme } = isTokenValid({ token });
    req.user = { name, userId, role, email, theme };
    next();
  } catch (error) {
...
how is a token created:
    ...
    const token = jwt.sign({ id: userId, username: username }, process.env.JWT_SECRET, {
    expiresIn: '30d', // Expires in 30 days
    });

{ id: userId, username: username } is the payload of the JWT, containing the user's ID and username.
...
2. **Payload**: Contains the claims or data being transferred, such as user ID, username, and any other information you want to include. It's also JSON-encoded and then base64url-encoded. For example: `eyJpZCI6IjEyMzQ1Njc4OSIsInVzZXJuYW1lIjoiZXhhbXBsZV91c2VyIn0`
...
In your example, when you create a JWT using `jwt.sign({ id: userId, username: username }, process.env.JWT_SECRET, { expiresIn: '30d' })`, the library handles the creation of the entire JWT, including the signature. So, the resulting string that represents the JWT is what you would typically refer to as the "token".
